...
# Tomcat 최신 버전 다운로드 (archive에서)
TOMCAT_DOWNLOAD_URL="https://archive.apache.org/dist/tomcat/tomcat-10/v10.1.30/bin/apache-tomcat-10.1.30.tar.gz"
...
wget --progress=bar:force:noscroll "$TOMCAT_DOWNLOAD_URL" -O apache-tomcat.tar.gz
...
if [ -d "$BACKEND_HOME/apache-tomcat-${TOMCAT_VERSION}" ]; then
    rm -rf "$BACKEND_HOME/apache-tomcat-${TOMCAT_VERSION}"
fi

tar -xzf apache-tomcat.tar.gz -C $BACKEND_HOME
...
echo ""
echo "접속:"
echo "  http://203.245.29.74:${TOMCAT_PORT}"
echo ""
echo "========================================"
