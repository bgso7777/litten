...
-- ================================================================================
-- ALTER: system_prompt 컬럼 추가 (DB 프롬프트 관리)
-- ================================================================================
ALTER TABLE `note_summary_config`
  ADD COLUMN `system_prompt` TEXT NULL
  COMMENT '요약 시스템 프롬프트. 플레이스홀더: {{LEVEL_DETAIL}} {{SOURCE_LANG}} {{OUTPUT_LANG}}. NULL=코드 fallback'
  AFTER `ai_model`;

ALTER TABLE `note_remind_config`
  ADD COLUMN `system_prompt` TEXT NULL
  COMMENT '리마인드 시스템 프롬프트 (요약 프롬프트 뒤에 합산). NULL=코드 fallback'
  AFTER `config_name`;

-- ================================================================================
-- note_system_prompt_config 테이블 생성 (요약/리마인드 프롬프트 통합 관리)
-- ================================================================================
DROP TABLE IF EXISTS `note_system_prompt_config`;
CREATE TABLE `note_system_prompt_config` (
  `sequence`      BIGINT(20)   NOT NULL AUTO_INCREMENT  COMMENT '설정 PK',
  `prompt_type`   VARCHAR(20)  NOT NULL                  COMMENT '프롬프트 유형 (summary|remind)',
  `file_type`     VARCHAR(50)  NOT NULL                  COMMENT '파일 유형 (youtube|text|pdf|...)',
  `summary_level` TINYINT      NULL                      COMMENT '요약 수준 1~5 (summary용; remind는 NULL)',
  `prompt_name`   VARCHAR(100) NOT NULL                  COMMENT '프롬프트 이름',
  `system_prompt` TEXT         NULL                      COMMENT '시스템 프롬프트. 플레이스홀더: {{LEVEL_DETAIL}} {{SOURCE_LANG}} {{OUTPUT_LANG}}',
  ...
  PRIMARY KEY (`sequence`) USING BTREE,
  UNIQUE INDEX `unique_prompt_type_file_level` (`prompt_type`, `file_type`, `summary_level`) USING BTREE,
  INDEX `idx_prompt_type_file` (`prompt_type`, `file_type`) USING BTREE,
  INDEX `idx_prompt_active`    (`is_active`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
  COMMENT='시스템 프롬프트 관리 (요약/리마인드 통합)';

-- note_summary_config, note_remind_config 에서 system_prompt 컬럼 제거
ALTER TABLE `note_summary_config` DROP COLUMN IF EXISTS `system_prompt`;
ALTER TABLE `note_remind_config`  DROP COLUMN IF EXISTS `system_prompt`;

-- ================================================================================
-- note_prompt_config 테이블 생성 (note_system_prompt_config 대체)
-- type: summary|remind / prompt_role: system|user / file_type / summary_level
-- ================================================================================
DROP TABLE IF EXISTS `note_system_prompt_config`;
DROP TABLE IF EXISTS `note_prompt_config`;
CREATE TABLE `note_prompt_config` (
  `sequence`      BIGINT(20)   NOT NULL AUTO_INCREMENT  COMMENT '설정 PK',
  ...
  `summary_level` TINYINT      NULL                      COMMENT '요약 수준 1~5 (summary용; remind는 NULL)',
  `prompt_name`   VARCHAR(100) NOT NULL                  COMMENT '프롬프트 이름',
  `prompt`        TEXT         NULL                      COMMENT '실제 프롬프트 내용. 플레이스홀더: {{LEVEL_DETAIL}} {{SOURCE_LANG}} {{OUTPUT_LANG}}',
  `is_active`     TINYINT(1)   NOT NULL DEFAULT 1         COMMENT '활성 여부',
  `description`   VARCHAR(500) NULL                      COMMENT '설명',
  `insert_pk`     BIGINT(20)   NULL                      COMMENT '등록자 FK',
  `update_pk`     BIGINT(20)   NULL                      COMMENT '수정자 FK',
  ...
  PRIMARY KEY (`sequence`) USING BTREE,
  UNIQUE INDEX `unique_type_role_file_level` (`type`, `prompt_role`, `file_type`, `summary_level`) USING BTREE,
  INDEX `idx_prompt_type_role_file` (`type`, `prompt_role`, `file_type`) USING BTREE,
  INDEX `idx_prompt_active`         (`is_active`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
  COMMENT='프롬프트 관리 (요약/리마인드, system/user 통합)';


-- ================================================================================
-- note_prompt_config 초기 데이터 삽입
-- summary/system 프롬프트: 8 유형 × 5 수준 = 40행
-- remind/system  프롬프트: 8 유형 = 8행
-- ================================================================================

-- ── 공통 요약 시스템 프롬프트 앞부분 (레벨·언어 플레이스홀더 포함) ──────────
...
-- ── 레벨별 출력 형식 섹션 ──────────────────────────────────────────────────
SET @p_lv1_output = '## 한줄 결론
(핵심 주제와 결론을 1~2문장으로)';

SET @p_lv2_output = '## 전체 목적 / 주제
(2~4문장)
...
-- 레벨 3·4·5 출력 형식 동일 (분량 지침은 LEVEL_DETAIL 플레이스홀더로 구분)
SET @p_lv3_output = '## 전체 목적 / 주제
(3~6문장으로 구체적으로 서술)
...
-- ── summary/system: 레벨 1 (8 유형) ──────────────────────────────────────
INSERT INTO `note_prompt_config`
  (`type`, `prompt_role`, `file_type`, `summary_level`, `prompt_name`, `prompt`, `description`)
SELECT 'summary', 'system', ft.file_type, 1,
       CONCAT(ft.file_type, ' 요약 Lv1 시스템 프롬프트'),
       CONCAT(@p_header, @p_lv1_output),
       '한줄 요약 (VERY_SHORT) 시스템 프롬프트'
FROM (SELECT 'youtube' AS file_type UNION ALL SELECT 'text' UNION ALL SELECT 'pdf' UNION ALL
      SELECT 'xls' UNION ALL SELECT 'doc' UNION ALL SELECT 'ppt' UNION ALL
      SELECT 'audio' UNION ALL SELECT 'handwriting') ft;

-- ── summary/system: 레벨 2 (8 유형) ──────────────────────────────────────
INSERT INTO `note_prompt_config`
  (`type`, `prompt_role`, `file_type`, `summary_level`, `prompt_name`, `prompt`, `description`)
SELECT 'summary', 'system', ft.file_type, 2,
       CONCAT(ft.file_type, ' 요약 Lv2 시스템 프롬프트'),
       CONCAT(@p_header, @p_lv2_output),
       '간단 요약 (SHORT) 시스템 프롬프트'
FROM (SELECT 'youtube' AS file_type UNION ALL SELECT 'text' UNION ALL SELECT 'pdf' UNION ALL
      SELECT 'xls' UNION ALL SELECT 'doc' UNION ALL SELECT 'ppt' UNION ALL
      SELECT 'audio' UNION ALL SELECT 'handwriting') ft;

-- ── summary/system: 레벨 3·4·5 (8 유형 × 3 수준 = 24행) ──────────────────
INSERT INTO `note_prompt_config`
  (`type`, `prompt_role`, `file_type`, `summary_level`, `prompt_name`, `prompt`, `description`)
SELECT 'summary', 'system', ft.file_type, lv.lv,
       CONCAT(ft.file_type, ' 요약 Lv', lv.lv, ' 시스템 프롬프트'),
       CONCAT(@p_header, @p_lv3_output),
       CONCAT('Lv', lv.lv, ' 요약 시스템 프롬프트')
FROM (SELECT 'youtube' AS file_type UNION ALL SELECT 'text' UNION ALL SELECT 'pdf' UNION ALL
      SELECT 'xls' UNION ALL SELECT 'doc' UNION ALL SELECT 'ppt' UNION ALL
      SELECT 'audio' UNION ALL SELECT 'handwriting') ft
CROSS JOIN (SELECT 3 AS lv UNION ALL SELECT 4 UNION ALL SELECT 5) lv;

-- ── remind/system: NULL level (8 유형) ────────────────────────────────────
INSERT INTO `note_prompt_config`
  (`type`, `prompt_role`, `file_type`, `summary_level`, `prompt_name`, `prompt`, `description`)
SELECT 'remind', 'system', ft.file_type, NULL,
       CONCAT(ft.file_type, ' 리마인드 시스템 프롬프트'),
       @p_remind,
       '리마인드 3단 계층 추출 시스템 프롬프트'
FROM (SELECT 'youtube' AS file_type UNION ALL SELECT 'text' UNION ALL SELECT 'pdf' UNION ALL
      SELECT 'xls' UNION ALL SELECT 'doc' UNION ALL SELECT 'ppt' UNION ALL
      SELECT 'audio' UNION ALL SELECT 'handwriting') ft;

-- 결과 확인
SELECT type, prompt_role, file_type, summary_level, prompt_name, is_active
FROM note_prompt_config
ORDER BY type, file_type, summary_level;
