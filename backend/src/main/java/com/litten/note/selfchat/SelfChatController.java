...
/**
 * '나와의 대화'(셀프 채팅) API. 로그인 회원 전용. 기기 간 동기화용.
 *  GET    /note/v1/self-chats                         내 방+항목 전체
 *  POST   /note/v1/self-chats                         방 생성/업서트 {name, clientId?}
 *  DELETE /note/v1/self-chats/{id}                    방 삭제
 *  POST   /note/v1/self-chats/{id}/messages           텍스트 추가 {content}
 *  POST   /note/v1/self-chats/{id}/files              파일 추가 (multipart: file, fileType, fileName, contentType)
 *  GET    /note/v1/self-chats/items/{itemId}/download 파일 다운로드
 */
@Log4j2
@RestController
...
public class SelfChatController {
    ...
    private final SelfChatService service;

    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @GetMapping("/note/v1/self-chats")
    ...
    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @PostMapping("/note/v1/self-chats")
    ...
    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @DeleteMapping("/note/v1/self-chats/{id}")
    ...
    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @PostMapping("/note/v1/self-chats/{id}/messages")
    ...
    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @PostMapping("/note/v1/self-chats/{id}/files")
