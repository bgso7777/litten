...
/**
 * 채팅 대화 숨김('방 나가기') API. 로그인 회원 전용. 다기기 동기화용.
 *  GET    /note/v1/hidden-conversations                   내 숨김 목록 [{convKey, hiddenAt}]
 *  POST   /note/v1/hidden-conversations                   숨김 {convKey}
 *  DELETE /note/v1/hidden-conversations?convKey=          숨김 해제
 */
@Log4j2
@RestController
...
public class HiddenConversationController {
    ...
    private final HiddenConversationService service;

    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @GetMapping("/note/v1/hidden-conversations")
    ...
    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @PostMapping("/note/v1/hidden-conversations")
    ...
    @CrossOrigin(origins = "*", allowedHeaders = "*")
    @DeleteMapping("/note/v1/hidden-conversations")
