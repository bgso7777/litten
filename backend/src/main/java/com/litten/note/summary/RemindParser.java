...
/**
 * AI 응답 텍스트에서 리마인드 섹션을 파싱하여 구조화된 데이터로 변환.
 *
 * AI 출력 포맷 기준:
 *   ─── 📌 리마인드 ───
 *   📂 [그룹명]
 ...
 */
@Log4j2
public class RemindParser {

    // 📌 리마인드 구분선 패턴 (앞뒤 공백, 대시 개수 무관하게 매칭)
    private static final Pattern REMIND_SEPARATOR =
            Pattern.compile("─+\\s*📌\\s*리마인드\\s*─+");
    ...
    public static class ParseResult {
        private final String summaryText;          // 리마인드 섹션 제거된 순수 요약 텍스트
        private final String fullText;             // 원본 전체 텍스트
        private final List<SummarySection> sections; // 요약 섹션 구조화 목록
        private final List<RemindGroup> groups;    // 리마인드 그룹 구조화 목록
        private final int totalCount;              // 리마인드 총 세부항목 수

        ParseResult(String summaryText, String fullText,
                    List<SummarySection> sections,
                    List<RemindGroup> groups, int totalCount) {
    ...
    /**
     * AI 응답 전체 텍스트를 파싱한다.
     * 구분선이 없으면 요약 텍스트만 반환하고 리마인드는 빈 리스트.
     */
    public static ParseResult parse(String fullText) {
        ...
        // 구분선 위치 탐색
        var matcher = REMIND_SEPARATOR.matcher(fullText);
        if (!matcher.find()) {
            log.debug("[RemindParser] 리마인드 구분선 없음 - 리마인드 파싱 생략");
        ...
        String remindText  = fullText.substring(matcher.end()).trim();

        log.debug("[RemindParser] 구분선 발견 - 요약 길이: {}, 리마인드 섹션 길이: {}",
                summaryText.length(), remindText.length());

        List<SummarySection> sections = parseSummarySections(summaryText);
        List<RemindGroup>    groups   = parseRemindSection(remindText);
        int totalCount = groups.stream().mapToInt(g -> g.getItems().size()).sum();

        log.info("[RemindParser] 파싱 완료 - 요약 섹션 수: {}, 그룹 수: {}, 총 세부항목 수: {}",
    ...
    private static List<SummarySection> parseSummarySections(String summaryText) {
        ...
        log.debug("[RemindParser] 요약 섹션 파싱 완료 - {}개", sections.size());
        return sections;
    }

    private static List<RemindGroup> parseRemindSection(String remindText) {
        List<RemindGroup> groups = new ArrayList<>();
        if (remindText.isBlank()) return groups;

        RemindGroup currentGroup = null;
        RemindItem  currentItem  = null;

        for (String raw : remindText.split("\n")) {
            ...
            if (line.startsWith(GROUP_PREFIX)) {
                ...
                currentItem = null;
                log.debug("[RemindParser] 그룹: {}", currentGroup.getGroupName());
                ...
                log.debug("[RemindParser] 항목 - type: {}, content: {}",
            ...
            } else if (line.contains("리마인드 총")) {
                // "리마인드 총 N개" 라인은 파싱에서 제외
                log.debug("[RemindParser] 총 개수 라인 스킵: {}", line.trim());
            }
    ...
    private static RemindItem parseItem(String line) {
