...
/**
 * 스터디룸 '방 나가기'(숨김) 상태 — 회원별로 대화 key를 숨긴 시각과 함께 저장한다.
 * 클라이언트는 대화의 최신 활동시각(lastAt)이 hidden_at 이하이면 목록에서 숨긴다.
 * 새 메시지/공유가 오면(lastAt > hidden_at) 자동으로 다시 보인다.
 * conv_key 예: 'u:이메일'(1:1), 'g:그룹명'(룸).
 */
@Getter
@Setter
@Entity(name = "HiddenRoom")
@Table(name = "note_hidden_room",
        uniqueConstraints = @UniqueConstraint(name = "uk_member_conv",
                columnNames = {"member_id", "conv_key"}))
@NoArgsConstructor
public class HiddenRoom extends BaseEntity implements Serializable {
